...
広告表示は `#PR` ではなく `[PR]`(ハッシュ記号なし)を使う。`#`付きだとThreadsが本文中の先頭のハッシュタグを「トピックタグ」としてユーザー名の隣にバッジ表示してしまう(`#`自体も本文表示から消える)ため、`[PR]`にして回避している(2026-09-18確認済み)。
...
- 2026-09-18: 表記を `#PR` → `[PR]` に変更(ハッシュ記号によるThreadsのトピックタグ化=ユーザー名隣へのバッジ表示を避けるため。詳細は上記「投稿の2段階構成」参照)。
